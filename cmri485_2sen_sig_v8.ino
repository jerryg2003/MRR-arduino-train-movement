...
uint8_t   servoSpeed = 20;                   //For VarSpeedServo
...
int lightType []     = {0, 1, 2, 0, 0};                   //0= red-green;  1= crossing;  2= crossing+gate
...
void setup() {
  ...
  bus.println("Setup complete: CMRI485_2Sen_Sig_v8 2019-12-06 2200");                 // *
  //                                                                                    *
  //*************************************************************************************
...
void CrossingGate(int k, int m, int n) {         //k=block number, m= entry sensor, n= exit sensor
  ...
  //TBD: allow potentiometer control as well as automatic control
  if (blockOcc[k])  {
    //Check if gate down
...
//  val = map(val, 0, 1023, 30, 77);       // scale it to use it with the servo (value from 0 and 180)
}
