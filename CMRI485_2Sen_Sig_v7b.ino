...
//Crossing signal can be triggered for each block
...
//v7 Added Crossing signals
//v7b Added full reset if JMRI bit 47 toggled
...
//   sensorPin = arduino pin number [Nano pins A1-A5 can be used as digital or analog pins;  A6-A7 are exclusively analog
//   sensorAnalog = true if analog, false if digital
...
int lightPin[]       = {8, 9, 10, 11, 6, 7, 0, 0, 0, 0};
int lightLowActive[] = {1, 0, 0, 0, 0};
...
void setup() {
  ...
  bus.println("Setup complete: CMRI485_2Sen_Sig_v7b 2019-11-04 0100");                 // *
  //                                                                                    *
  //*************************************************************************************
...
void loop() {
  ...
  for (int i = 0; i < numBlocks; i++)   {
    ...
    //==================== Turn on crossing signal if block occupied ===========================
  ...
  if (blockOcc[k])  {
    if (crossingSignalTime [k] < curTime)   {                //If time to alternate...
      if (crossingSignalAlt[k])  {
        ...
        digitalWrite(lightPin[m], HIGH ^ lightLowActive[k]); //Alternate on-off left-right
        digitalWrite(lightPin[n], LOW ^ lightLowActive[k]);
        crossingSignalTime [k] = curTime + crossingWait;     //Set time to wait to alternate
      }
      else  {
        crossingSignalAlt [k] = 1;
        digitalWrite(lightPin[n], HIGH ^ lightLowActive[k]); //Alternate on-off right-left
        digitalWrite(lightPin[m], LOW ^ lightLowActive[k]);
        crossingSignalTime [k] = curTime + crossingWait;     //Set time to wait to alternate
      }
    }
  }
